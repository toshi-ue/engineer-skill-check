...
class EmployeesController < ApplicationController
  ...
  def index
    @pagy, @employees = pagy(Employee.active.order("#{sort_column} #{sort_direction}"))
  end
  ...
  def employee_params
    params.require(:employee).permit(:number, :last_name, :first_name, :account, :password, :email,
                                     :email_confirmation, :date_of_joining, :department_id, :office_id, :employee_info_manage_auth, :new_posting_auth)
  end
